...
?>
<?php $this->load->view('gallery/inc/header', $includes); ?>
...
<div class="w100" style="margin-bottom: 10px;">
  ...
  <ul class="pager">
    <li class="previous">
      <a href="<?php echo site_url('gallery/album'); ?>">&larr; Back to albums</a>
    </li>
  </ul>
  ...
  <div class="well">
    ...
    <p id="upload-btn" style="margin:10px 0;">
      <a href="#" class="uploadifyUpload btn btn-primary btn-large">Upload Files</a>
    </p>
    <div id="new-images">
      <h4>Uploaded Images</h4>
      <p><a class="btn" href="<?php echo site_url("gallery/album/images/$album->id"); ?>" style="margin: 10px 0;"><i class="icon-refresh"></i> Refresh</a></p>
  ...
  <ul id="sortable">
    ...
    <li id="image_<?php echo $image->id; ?>" class="ui-state-default" style="height: <?php echo $config->thumb_height + 10; ?>px">
      ...
      <div class="info" style="left: <?php echo $config->thumb_width + 50; ?>px">
        ...
        Caption: 
          <?php if (empty($image->caption)): ?>
            <a href="<?php echo site_url("gallery/image/edit/$album->id/$image->id"); ?>">Create one</a>
          <?php else: ?>
      ...
      <div class="btn-group">
        ...
        <a href="<?php echo site_url("gallery/image/download/$image->id"); ?>" class="btn" title="Download" rel="tooltip" data-original-title="Download"><i class="icon-download-alt"></i></a>
        <a href="<?php echo site_url("gallery/image/edit/$album->id/$image->id"); ?>" class="btn" title="Edit" rel="tooltip" data-original-title="Edit"><i class="icon-pencil"></i></a>
        <?php /* <a href="<?php echo site_url("gallery/image/comments/$album->id/$image->id"); ?>" class="btn" title="Comments" rel="tooltip" data-original-title="Comments"><i class="icon-comment"></i></a> */ ?>
        <?php if ($image->published == 1): ?>
        <a href="<?php echo site_url("gallery/image/unpublish/$album->id/$image->id"); ?>" class="btn btn-success" title="Published" rel="tooltip" data-original-title="Published"><i class="icon-ok icon-white"></i></a>
        <?php else: ?>
        <a href="<?php echo site_url("gallery/image/publish/$album->id/$image->id"); ?>" class="btn" title="Unpublished" rel="tooltip" data-original-title="Unpublished"><i class="icon-ok"></i></a>
        <?php endif; ?>
        <a href="#image-modal" class="btn btn-danger image-delete-btn" title="Delete" rel="tooltip" action="<?php echo site_url("gallery/image/remove/$album->id/$image->id"); ?>" data-toggle="modal" data-original-title="Delete"><i class="icon-remove icon-white"></i></a>
      </div>
    </li>
    <?php endforeach; ?>
  </ul>
...
<span class="right w20">
  <div class="well sidebar-nav">
    <ul class="nav nav-list">
      ...
      <li><a href="<?php echo site_url("gallery/album/edit/$album->id"); ?>"><i class="icon-pencil"></i>Rename</a></li>
      <li><a href="<?php echo site_url("gallery/album/configure/$album->id"); ?>"><i class="icon-cog"></i>Configure</a></li>
      <li><a href="<?php echo site_url("gallery/api/feed/json/$album->uuid"); ?>" target="_blank"><i class="icon-book"></i>JSON Feed</a></li>
      <li><a href="<?php echo site_url("gallery/api/feed/xml/$album->uuid"); ?>" target="_blank"><i class="icon-book"></i>XML Feed</a></li>
...
$(document).ready(function() {
  ...
  $("#sortable").sortable({
    ...
    update : function () { 
      ...
      $.ajax({
        url          : '<?php echo base_url(); ?>gallery/album/reorder?' + order,
        type         : 'GET',
        cache        : false,
  ...
  $('#file_upload').uploadify({
    'swf'       : '<?php echo base_url(); ?>assets/flash/uploadify.swf',
    'uploader'         : '<?php echo base_url(); ?>gallery/api/upload/<?php echo $album->id; ?>',
//    'buttonImage'      : '<?php echo base_url(); ?>assets/images/cancel.png',
    'auto'           : false,
    'multi'          : true,
...
//    'debug'          : true,
    ...
    'onUploadSuccess'     : function(fileObj, data, response) {
      ...
      $('#new-images').show();
      $.ajax({
        url          : '<?php echo base_url(); ?>gallery/album/resize/<?php echo $album->id; ?>/' + data,
        type         : 'POST',
        cache        : false,
        success      : function(response) {
          if (response !== 'failure') {
  ...
  $('.uploadifyUpload').click(function(e) {
    e.preventDefault();
...
<?php $this->load->view('gallery/inc/footer'); ?>
